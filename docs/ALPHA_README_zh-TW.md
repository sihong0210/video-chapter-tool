# 直播影片摘要工具 0.2.0 Alpha 2（Portable）

這是供內部測試的 Windows 10／11 64 位元 one-folder 版本。程式已隨附 Python 與執行元件，使用電腦不需要另外安裝 Python；請保留整個資料夾，不可只複製 `VideoChapterTool.exe`。

## 啟動

1. 將 ZIP 完整解壓縮到一般可寫入的資料夾。
2. 執行 `VideoChapterTool.exe`。
3. 到「錄製與儲存設定」選擇模型與匯出資料夾。
...
- 新版本 ZIP 不含 `UserData`。解壓縮後可用新的程式檔與 `_internal` 覆蓋舊版本，既有 `UserData` 必須保留；更保守的方式是解壓到新資料夾，再把舊 `UserData` 整個移入。
...
- 從目前開發用 `.runtime` 延續資料時，應先以新版程式開啟該資料目錄一次完成路徑升級，關閉後再把 `.runtime` 的完整內容複製為 Portable 的 `UserData`。
...
1. 按「重新檢測」。
2. 按「匯出診斷包 ZIP」。
3. 將 ZIP 提供給維護者；它只包含版本、硬體診斷與已遮蔽敏感資訊的輪替日誌，不含 SQLite、原始設定檔、逐字稿或音訊。
...
- AMD GPU 目前仍使用 CPU 路徑；ROCm／HIP 加速不在本 Alpha 範圍。
