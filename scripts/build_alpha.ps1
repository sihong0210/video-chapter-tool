...
$appFolder = Join-Path $releaseRoot "VideoChapterTool"
$version = "0.2.0a2"
$archive = Join-Path $releaseRoot "VideoChapterTool-$version-alpha-win64.zip"
$alphaGuide = Join-Path $repoRoot "docs\ALPHA_README_zh-TW.md"
$releaseNotes = Join-Path $repoRoot "docs\RELEASE_NOTES_0.2.0a2_zh-TW.md"
...
try {
    ...
    if (-not $SkipTests) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Automated tests failed; Alpha build stopped."
        }
    }

    & $python -m PyInstaller --noconfirm --clean --distpath $releaseRoot --workpath $workRoot $spec
    if ($LASTEXITCODE -ne 0) {
    ...
    Copy-Item -LiteralPath $alphaGuide -Destination (Join-Path $appFolder "ALPHA_README_zh-TW.md") -Force
    ...
    Write-Host "Alpha build completed: $exePath"
